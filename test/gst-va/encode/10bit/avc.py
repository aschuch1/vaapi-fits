...
from .....lib.gstreamer.va.encoder import AVC_10EncoderTest, AVC_10EncoderLPTest
...
class cqp(AVC_10EncoderTest):
  ...
  @slash.parametrize(*gen_avc_cqp_parameters(spec))
  ...
  @slash.parametrize(*gen_avc_cqp_parameters(spec_r2r))
...
class cbr(AVC_10EncoderTest):
  ...
  @slash.parametrize(*gen_avc_cbr_parameters(spec))
  ...
  @slash.parametrize(*gen_avc_cbr_parameters(spec_r2r))
...
class vbr(AVC_10EncoderTest):
  ...
  @slash.parametrize(*gen_avc_vbr_parameters(spec))
  def test(self, case, gop, bframes, tilecols, tilerows, bitrate, fps, quality, profile):
    self.init(spec, case, gop, bframes, tilecols, tilerows, bitrate, fps, quality, profile)
    self.encode()

  @slash.parametrize(*gen_avc_vbr_parameters(spec_r2r))
...
class cqp_lp(AVC_10EncoderLPTest):
  ...
  @slash.parametrize(*gen_avc_cqp_lp_parameters(spec))
  def test(self, case, gop, bframes, tilecols, tilerows, qp, quality, profile):
    self.init(spec, case, gop, bframes, tilecols, tilerows, qp, quality, profile)
    self.encode()

  @slash.parametrize(*gen_avc_cqp_lp_parameters(spec_r2r))
  def test_r2r(self, case, gop, bframes, tilecols, tilerows, qp, quality, profile):
    self.init(spec_r2r, case, gop, bframes, tilecols, tilerows, qp, quality, profile)
    vars(self).setdefault("r2r", 5)
    self.encode()
...
class cbr_lp(AVC_10EncoderLPTest):
  ...
  @slash.parametrize(*gen_avc_cbr_lp_parameters(spec))
  def test(self, case, gop, bframes, tilecols, tilerows, bitrate, quality, fps, profile):
    self.init(spec, case, gop, bframes, tilecols, tilerows, bitrate, fps, quality, profile)
    self.encode()

  @slash.parametrize(*gen_avc_cbr_lp_parameters(spec_r2r))
  def test_r2r(self, case, gop, bframes, tilecols, tilerows, bitrate, quality, fps, profile):
    self.init(spec_r2r, case, gop, bframes, tilecols, tilerows, bitrate, fps, quality, profile)
    vars(self).setdefault("r2r", 5)
    self.encode()
...
class vbr_lp(AVC_10EncoderLPTest):
  ...
  @slash.parametrize(*gen_avc_vbr_lp_parameters(spec))
  def test(self, case, gop, bframes, tilecols, tilerows, bitrate, fps, quality, profile):
    self.init(spec, case, gop, bframes, tilecols, tilerows, bitrate, fps, quality, profile)
    self.encode()

  @slash.parametrize(*gen_avc_vbr_lp_parameters(spec_r2r))
  def test_r2r(self, case, gop, bframes, tilecols, tilerows, bitrate, fps, quality, profile):
    self.init(spec_r2r, case, gop, bframes, tilecols, tilerows, bitrate, fps, quality, profile)
    vars(self).setdefault("r2r", 5)
    self.encode()
